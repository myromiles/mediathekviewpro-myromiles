...
const CATEGORY_MAP = {
    ...
    "Talk & Show": "Lanz, Miosga, Maischberger, Illner, Hart aber fair",
    "Comedy & Satire": "heute-show, extra 3, Anstalt, Magazin Royale"
};
...
async function fetchItems(genre, channel) {
    const searchTerm = genre ? CATEGORY_MAP[genre] : "a";
    const payload = {
        queries: [
            { fields: ["title", "topic"], query: searchTerm },
            ...(channel !== "all" ? [{ fields: ["channel"], query: channel.toLowerCase() }] : [])
        ],
        sortBy: "timestamp",
        sortOrder: "desc",
        size: 50
