...
const SHOW_DATABASE = {
  ...
  "tagesschau": { host: "Tagesschau-Team", category: "Nachrichten", keywords: ["tagesschau", "tagesthemen", "in go undest"] },
...
const MANIFEST = {
  ...
  description: "Erkennt Moderatoren, Sendungen & Kategorien automatisch. Developed by MyroMiles.",
...
// Helper 1: API-Abfrage
async function fetchSmartMediathekItems({ query = "", channel = "", genre = "", limit = 30 }) {
  const queries = [];
...
// Helper 2: SMART MODERATOR & CATEGORY DETECTOR
function analyzeItem(item) {
  ...
  // 1. Prüfe auf Moderatoren & feste Sendungen
  ...
  // 2. Allgemeine Kategorie-Erkennung
...
app.get("/catalog/:type/:id/:extra?.json", async (req, res) => {
  ...
  const metas = await Promise.all(
    items.map(async (item) => {
      ...
      // Nutze die Analyse für Moderatoren & Genres
...
app.listen(PORT, () => console.log(`MediathekViewPro v2.1.0 Smart-Matrix läuft auf Port ${PORT}`));